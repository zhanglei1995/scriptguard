...
export async function createRepeatableTestRun(config: ScheduleConfig): Promise<string> {
  ...
  console.log(
    `[Scheduler] Created repeatable job for schedule ${config.scheduleId}: ${config.cron}`,
  );
  return job.id!;
}

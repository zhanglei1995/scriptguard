...
export const syncRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    ...
    async (req, reply) => {
      ...
      if (changes.deletedScriptIds && changes.deletedScriptIds.length > 0) {
        const validIds = changes.deletedScriptIds.filter(
  ...
  fastify.post(
    ...
    async (req) => {
      ...
      const allChanged = await db
        .select()
        .from(scripts)
        .where(and(eq(scripts.userId, userId), gt(scripts.updatedAt, lastSyncDate)));

      const deletedScriptIds = allChanged.filter((s) => s.deletedAt !== null).map((s) => s.id);

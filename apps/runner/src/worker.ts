...
const logger = pino({
  ...
  base: { service: 'scriptguard-runner' },
});
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      window.addEventListener('error', (e: ErrorEvent) => {
        (window as any).__scriptguard_errors.push({
          type: 'error',
          message: e.message,
          stack: e.error?.stack,
        });
    ...
    const injectedErrors = await page
      .evaluate(() => {
    ...
    const requiredRules = job.data.rules.filter(
      (r) => (r.config as { required?: boolean }).required !== false,
    );

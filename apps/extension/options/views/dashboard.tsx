...
export function DashboardTab() {
  ...
  const load = useCallback(async () => {
    const [checks, alerts] = await Promise.all([getRecentChecks(10), getUnacknowledgedAlerts()]);
  ...
  return (
    <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <span className="text-sm font-medium">{script?.name ?? check.scriptId}</span>
      ...
      <div className="flex gap-3">
        <Button variant="outline" onClick={() => alert('添加脚本功能即将上线')}>
          <Plus className="mr-2 h-4 w-4" />
          添加脚本
        </Button>
        <Button variant="outline" onClick={() => alert('查看告警功能即将上线')}>
          <Bell className="mr-2 h-4 w-4" />
          查看告警
        </Button>
        <Button variant="outline" onClick={() => alert('运行全部测试功能即将上线')}>

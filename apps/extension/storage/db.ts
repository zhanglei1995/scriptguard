...
import type { HealthStatus, AlertLevel } from './schemas'
...
// ====== Alerts ======
export async function addAlert(record: Omit<AlertRecord, 'id' | 'acknowledged'> & { acknowledged?: boolean }): Promise<number> {
  const db = getDB()
...
// ====== Cleanup ======
export async function cleanupOldRecords(): Promise<{ checks: number; snapshots: number; alerts: number }> {
  const db = getDB()

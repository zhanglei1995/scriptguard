...
// ====== Page Check State ======
export async function setPageCheckState(tabId: number, state: Omit<PageCheckState, 'tabId'>): Promise<void> {
...
export async function setCommToken(tabId: number, token: string, ttlMs: number = 60000): Promise<void> {
  const now = Date.now()

...
describe('Chrome Storage', () => {
  describe('scriptsStore', () => {
    ...
    it('validates data on set', async () => {
      await expect(
        scriptsStore.set([{ id: 123 } as any])
      ).rejects.toThrow()
    })
  ...
  describe('rulesStore', () => {
    it('sets and gets rules', async () => {
      const rules = [
        { id: '1', scriptId: 's1', type: 'selector' as const, target: '.btn', operator: 'exists' as const, enabled: true },
      ]
      await rulesStore.set(rules)
  ...
  describe('schedulesStore', () => {
    it('sets and gets schedules', async () => {
      const schedules = [
        { id: '1', scriptId: 's1', intervalMinutes: 30, enabled: true },
      ]
  ...
  describe('channelsStore', () => {
    it('sets and gets channels', async () => {
      const channels = [
        { id: '1', type: 'browser' as const, enabled: true, config: {} },
      ]

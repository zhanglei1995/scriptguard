...
describe('RuleEngine', () => {
  ...
  describe('executeAll', () => {
    ...
    it('returns degraded when optional rule fails', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'missing', type: 'selector_exists', config: { selector: '#optional' }, required: false },
      ]
    ...
    it('stops on failure when continueOnError is false', async () => {
      const rules: CheckRule[] = [
        { id: 'r1', name: 'fail', type: 'selector_exists', config: { selector: '#missing' }, required: true },
        { id: 'r2', name: 'success', type: 'url_match', config: { pattern: 'https://example.com/*' }, required: true },
      ]

...
describe('Integration: MVP Rule Executors', () => {
  ...
  describe('multi-rule combinations', () => {
    it('all rules pass - healthy status', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'title visible', type: 'selector_visible', config: { selector: '#title' }, required: true },
        { id: 'r3', name: 'status text', type: 'text_content', config: { selector: '#status', expected: 'Active', operator: 'equals' }, required: true },
    ...
    it('required rule fails - failed status', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'missing element', type: 'selector_exists', config: { selector: '#does-not-exist' }, required: true },
        { id: 'r2', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
      ]
    ...
    it('optional rule fails - degraded status', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'missing optional', type: 'selector_exists', config: { selector: '#optional' }, required: false },
      ]
    ...
    it('all optional rules fail - degraded status', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'missing opt1', type: 'selector_exists', config: { selector: '#opt1' }, required: false },
        { id: 'r3', name: 'missing opt2', type: 'text_content', config: { selector: '#opt2', expected: 'test' }, required: false },
      ]
  ...
  describe('js_assertion integration', () => {
    it('passes js_assertion combined with other rules', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'count check', type: 'js_assertion', config: { expression: 'document.querySelector("#count").textContent === "5"' }, required: true },
      ]
    ...
    it('js_assertion failure causes failed status', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'wrong count', type: 'js_assertion', config: { expression: 'document.querySelector("#count").textContent === "10"' }, required: true },
      ]
  ...
  describe('console_clean integration', () => {
    ...
    it('passes with ignore patterns', async () => {
      ...
      const ctx = createCtx(doc, {
        capturedErrors: [
          { source: 'window', type: 'error', message: 'Third-party script failed', timestamp: Date.now() },
        ],
      })

      const rules: CheckRule[] = [
        { id: 'r1', name: 'console clean', type: 'console_clean', config: { ignorePatterns: ['Third-party'] }, required: true },
      ]
  ...
  describe('duration integration', () => {
    it('passes when load time within threshold', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'duration', type: 'duration', config: { maxDuration: 2000 }, required: true },
      ]
    ...
    it('fails when load time exceeds threshold', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'duration', type: 'duration', config: { maxDuration: 3000 }, required: true },
      ]
  ...
  describe('timeout scenarios', () => {
    it('handles overall engine timeout', async () => {
      ...
      const rules: CheckRule[] = [
        ...
        { id: 'r2', name: 'url', type: 'text_content', config: { selector: '#app', expected: 'App' }, required: true },
      ]
    ...
    it('rule timeout returns timeout status', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'slow rule', type: 'selector_exists', config: { selector: '#app' }, required: true },
      ]
  ...
  describe('error handling', () => {
    ...
    it('continues after error when continueOnError is true', async () => {
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'unknown', type: 'unknown_type' as RuleType, config: {}, required: true },
        { id: 'r2', name: 'exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
      ]
  ...
  describe('mixed executor types', () => {
    it('combines all 6 executor types in one run', async () => {
      ...
      const ctx = createCtx(doc, {
        window: { performance: mockPerf, getComputedStyle: () => ({ display: 'block', visibility: 'visible' }) } as unknown as Window,
        capturedErrors: [],
      })
      ...
      const rules: CheckRule[] = [
        { id: 'r1', name: 'app exists', type: 'selector_exists', config: { selector: '#app' }, required: true },
        { id: 'r2', name: 'title visible', type: 'selector_visible', config: { selector: '#title' }, required: true },
        { id: 'r3', name: 'status text', type: 'text_content', config: { selector: '#status', expected: 'OK', operator: 'equals' }, required: true },
        { id: 'r4', name: 'js check', type: 'js_assertion', config: { expression: 'document.querySelectorAll("div").length > 0' }, required: true },
        { id: 'r5', name: 'no errors', type: 'console_clean', config: {}, required: true },
        { id: 'r6', name: 'fast load', type: 'duration', config: { maxDuration: 2000 }, required: true },
      ]

...
describe('LogStore', () => {
  ...
  describe('getChecks', () => {
    beforeEach(async () => {
      // Add test data
      await store.addCheck(createCheckRecord({ scriptId: 'script-a', status: 'healthy', timestamp: new Date('2024-01-01') }))
      await store.addCheck(createCheckRecord({ scriptId: 'script-b', status: 'failed', timestamp: new Date('2024-01-02') }))
      await store.addCheck(createCheckRecord({ scriptId: 'script-a', status: 'failed', timestamp: new Date('2024-01-03') }))
      await store.addCheck(createCheckRecord({ scriptId: 'script-c', status: 'healthy', timestamp: new Date('2024-01-04') }))
    })

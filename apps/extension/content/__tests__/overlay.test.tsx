...
describe('StatusOverlay', () => {
  ...
  describe('degraded state', () => {
    it('renders degraded icon and label', () => {
      render(
        <StatusOverlay
          {...makeProps({ status: 'degraded', failedRules: ['selector-missing'] })}
        />
      )
    ...
    it('shows failed rules', () => {
      render(
        <StatusOverlay
          {...makeProps({ status: 'degraded', failedRules: ['rule-a', 'rule-b'] })}
        />
      )
    ...
    it('shows mute button when onMute provided', () => {
      ...
      render(
        <StatusOverlay
          {...makeProps({ status: 'degraded', failedRules: ['r1'], onMute })}
        />
      )
    ...
    it('does not auto-hide', () => {
      ...
      render(
        <StatusOverlay
          {...makeProps({ status: 'degraded', failedRules: ['r1'], onClose })}
        />
      )
      act(() => {
  ...
  describe('failed state', () => {
    it('renders failed icon and label', () => {
      render(
        <StatusOverlay
          {...makeProps({ status: 'failed', failedRules: ['r1'] })}
        />
      )
      expect(screen.getByText('🔴')).toBeDefined()
    ...
    it('shows disable button when onDisable provided', () => {
      ...
      render(
        <StatusOverlay
          {...makeProps({ status: 'failed', failedRules: ['r1'], onDisable })}
        />
      )
    ...
    it('does not auto-hide', () => {
      ...
      render(
        <StatusOverlay
          {...makeProps({ status: 'failed', failedRules: ['r1'], onClose })}
        />
      )
      act(() => {
        vi.advanceTimersByTime(5000)

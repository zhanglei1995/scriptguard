...
class TestRunner {
  ...
  async runJob(job: Job<TestJobData>) {
    ...
    try {
      ...
      // 等待脚本执行
      await page.waitForFunction(() => (window as any).__SG_REPORT__ !== undefined, { timeout });
...
class WebhookChannel implements NotificationChannel {
  constructor(
    private config: { url: string; template: 'feishu' | 'dingtalk' | 'slack' | 'generic' },
  ) {}
  async send(payload: NotificationPayload) {
        ...
        "actions": [
          {
            ...
            "url": "https://scriptguard.dev/scripts/script_001"
          }
        ]
      }
    ]
...
_ScriptGuard TDD v1.0 · 2026-06-17 · 与 PRD v1.0-merged 配套使用_
